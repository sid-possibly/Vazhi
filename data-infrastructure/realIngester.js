...
const path = require('path');
const { parse } = require('csv-parse/sync');
const graphService = require('../Intelligence_platform/services/graphService.js');

/**
 * Normalizes GTFS time strings (e.g. 25:10:00 -> 01:10:00)
 */
const formatGTFSTime = (timeStr) => {
  if (!timeStr) return null;
  const parts = timeStr.trim().split(':');
...
const ingestGTFSForMode = async (pool, citySlug, modeType, folderPath, agencyFilter = []) => {
  ...
  try {
    ...
    if (metaRes.rows.length === 0) {
      throw new Error(`No DB record found for city '${citySlug}' and mode '${modeType}'. Ensure transport_modes table is populated.`);
    }
    ...
    const routeMap = new Map();
    // Debug: Collect all unique agency IDs in the file to help the teammate
    const foundAgencies = new Set(routes.map(r => r.agency_id));
    
    for (const r of routes) {
      // Robust Check: Trim the agency_id from the file before comparing
      const agencyInFile = (r.agency_id || '').trim();
      
      if (agencyFilter.length > 0 && !agencyFilter.includes(agencyInFile)) continue;
    ...
    if (routeMap.size === 0) {
      console.warn(`   ⚠️  No routes matched for [${modeType}]. Agencies in file: [${Array.from(foundAgencies).join(', ')}]`);
    } else {
      console.log(`   ✅ Routes ingested: ${routeMap.size}`);
    }
    ...
    // ── 3. TRIPS & SCHEDULES ─────────────────────────────────────────────────
    const trips = parse(fs.readFileSync(path.join(folderPath, 'trips.txt')), { columns: true });
    const tripRouteMap = new Map();
    for (const t of trips) {
      if (routeMap.has(t.route_id)) tripRouteMap.set(t.trip_id, routeMap.get(t.route_id));
    }

    const stopTimes = parse(fs.readFileSync(path.join(folderPath, 'stop_times.txt')), { columns: true });
    let schedulesInserted = 0;
    ...
    for (const st of stopTimes) {
      ...
      await client.query(
        `INSERT INTO schedules (stop_id, route_id, trip_id, arrival_time, departure_time, stop_sequence)
         ...
         FROM stops s
         WHERE s.gtfs_stop_id = $6 AND s.city_id = $7
         LIMIT 1 ON CONFLICT DO NOTHING`,
        [dbRouteId, st.trip_id, formatGTFSTime(st.arrival_time), formatGTFSTime(st.departure_time), parseInt(st.stop_sequence), st.stop_id, city_id]
      );
      schedulesInserted++;
    }

    await client.query('COMMIT');
    graphService.clearCache(city_id); // Phase 1 Task 4 Fix

    // Feed Metadata Record
    await pool.query(
      `INSERT INTO gtfs_feed_metadata (city_id, mode_id, last_ingested_at, routes_count, stops_count, schedules_count)
       VALUES ($1, $2, NOW(), $3, $4, $5)
       ON CONFLICT (city_id, mode_id) DO UPDATE SET last_ingested_at = NOW()`,
      [city_id, mode_id, routeMap.size, stops.length, schedulesInserted]
    );
    ...
    await client.query('ROLLBACK');
    console.error(`❌ [${modeType}] Ingestion failed:`, err.message);
    throw err;
  } finally {
    client.release();
  }
